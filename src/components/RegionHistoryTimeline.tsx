...
export function RegionHistoryTimeline({
  ...
  return (
    <div className={cn('relative', className)}>
      ...
      <ol className="relative space-y-0 border-l-2 border-(--line) pl-5">
        {events.map((event, index) => (
          <li key={`${event.year}-${event.title}-${index}`} className="relative pb-6 last:pb-0">
            <span
              className="absolute left-[1.35rem] top-1 flex size-3 rounded-full border-2 border-(--lagoon-deep) bg-white"
              aria-hidden
            />
            <time
